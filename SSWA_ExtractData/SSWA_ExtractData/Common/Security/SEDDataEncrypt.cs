...
namespace SSWA_ExtractData.Common.Security
{
    ...
    public class SEDDataEncrypt
    {
        ...
        private string EncryptionByMD5(string input)
        {
            ...
            foreach (var b in bs)
            {
                s.Append(b.ToString(SEDConst.X_2_LOWER).ToLower());
            }
            return s.ToString();
        }
        ...
        {
            var strChangeSha = EncryptionBySHA512(input);
        ...
        {
            var arrayCharData = input.ToArray();
            var sbdEncode = new StringBuilder();
            foreach (var item in arrayCharData)
            {
                sbdEncode.Append(EncryptionOneSt(item));
            }
            var strEncodeOutput = new StringBuilder(SEDConst.HEAD_ANDERS)
                .Append(sbdEncode).Append(SEDConst.END_HEJLSBERG).ToString();
        ...
        private string DecodeOneSt(string headerInput, string input)
        {
            ...
            if (headerInput.Equals(SEDConst.HEAD_W))
            {
                return input.Substring(2, 1);
            }
            if (headerInput.Equals(SEDConst.HEAD_E))
            {
                return input.Substring(3, 1);
            }
            if (headerInput.Equals(SEDConst.HEAD_R))
            {
                return input.Substring(4, 1);
            }
            if (headerInput.Equals(SEDConst.HEAD_T))
            {
        ...
        public string DecodeTwoWay(string input)
        {
            ...
            for (var i = 0; i < arrayEncode.Length; i++)
            {
                ...
                sbdDecode.Append(DecodeOneSt(headerEncode, arrayEncode[i]));
            }
